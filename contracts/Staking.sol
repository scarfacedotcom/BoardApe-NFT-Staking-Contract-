...
contract BoardApeStaking {
    ...
    function stake(uint256 _amount) external {
        ...
        require(
            IERC20(platformToken).balanceOf(msg.sender) >= _amount,
            "Insufficient balance"
        );
        require(
            IERC721(boardApeNFT).balanceOf(msg.sender) > 0,
            "You must own a Board Ape NFT"
        );

        IERC20(platformToken).transferFrom(msg.sender, address(this), _amount);

        StakedData storage stakedData = stakes[msg.sender];
        uint timeSinceLastCalculation = block.timestamp -
            stakedData.lastInterestCalculation;
    ...
    function withdraw() external {
        ...
        require(
            ...
            "You currently do not have any stake"
        );
        require(
            IERC20(platformToken).balanceOf(address(this)) >=
                stakedData.stakedAmount,
            "Insufficient funds"
        );
    ...
    function calculateWithdrawalAmount() public view returns (uint) {
        ...
        require(
            IERC20(platformToken).balanceOf(address(this)) >= balance,
            "Insufficient funds"
        );
